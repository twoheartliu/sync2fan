# PWA (Progressive Web App) 功能说明

✅ **Web App Manifest** - `/public/manifest.json`
- 应用名称、图标、主题色等配置
- 支持 standalone 模式（独立应用窗口）

✅ **Service Worker** - `/public/service-worker.js`
- 离线缓存支持
- 网络优先策略
- 自动更新检测

✅ **PWA Meta 标签** - `index.html`
- Manifest 引用
- Apple Touch Icon 支持
- 主题色配置

✅ **Service Worker 注册** - `src/main.js`
- 仅在生产环境启用
- 自动更新提示

✅ **应用图标** - `/public/icons/`
- SVG 源文件
- 图标生成工具

## 生成应用图标

### 方法 1：使用浏览器生成器（推荐）

1. 启动开发服务器：
   ```bash
   npm run dev
   ```

2. 在浏览器中访问：
   ```
   http://localhost:1234/icons/generator.html
   ```

3. 点击"生成图标"按钮

4. 下载所有生成的图标到 `public/icons/` 目录

### 方法 2：使用在线工具

1. 访问 [PWA Icon Generator](https://www.pwabuilder.com/imageGenerator)
2. 上传 `/public/icons/icon.svg`
3. 下载并解压图标包
4. 将图标复制到 `public/icons/` 目录

### 方法 3：使用命令行工具

如果安装了 ImageMagick：

```bash
cd public/icons
for size in 72 96 128 144 152 192 384 512; do
  convert icon.svg -resize ${size}x${size} icon-${size}x${size}.png
done
```

## 测试 PWA 功能

### 开发环境测试

由于 Service Worker 仅在生产环境启用，需要先构建项目：

```bash
# 构建生产版本
npm run build

# 预览生产版本
npm run preview
```

### Chrome 桌面端测试

1. 访问应用 URL
2. 点击地址栏右侧的 "安装" 图标 ⊕
3. 确认安装应用
4. 应用将作为独立窗口打开

### Chrome 移动端测试

1. 在手机浏览器中访问应用
2. 点击菜单中的"添加到主屏幕"
3. 应用图标将出现在主屏幕上
4. 点击图标以全屏模式打开应用

### 开发者工具检查

1. 打开 Chrome DevTools (F12)
2. 切换到 **Application** 标签
3. 检查以下项目：
   - **Manifest**: 查看 manifest.json 配置
   - **Service Workers**: 查看 SW 状态和缓存
   - **Cache Storage**: 查看缓存内容

### 离线功能测试

1. 访问应用并等待缓存完成
2. 在 DevTools Network 面板中选择 "Offline"
3. 刷新页面，应用仍可访问（显示缓存内容）

## PWA 功能特性

### ✨ 安装到设备
- 桌面：添加到应用列表
- 移动：添加到主屏幕
- 独立窗口运行（无浏览器地址栏）

### 📡 离线支持
- 网络优先策略
- 失败时使用缓存
- 关键资源预缓存

### 🔄 自动更新
- 检测新版本
- 提示用户刷新
- 无缝更新体验

### 🎨 原生体验
- 自定义主题色
- 启动画面
- 全屏显示

## 部署注意事项

### HTTPS 要求
- Service Worker 需要 HTTPS（或 localhost）
- 确保生产环境使用 HTTPS

### 缓存策略
- 当前使用网络优先策略
- 可根据需求调整为缓存优先或其他策略

### 图标要求
- 至少需要 192x192 和 512x512 尺寸
- 建议提供多种尺寸以适配不同设备
- 支持 maskable 图标（Android 自适应图标）

## 自定义配置

### 修改应用信息

编辑 `/public/manifest.json`：
```json
{
  "name": "你的应用名称",
  "short_name": "短名称",
  "description": "应用描述",
  "theme_color": "#你的主题色",
  "background_color": "#你的背景色"
}
```

### 修改缓存策略

编辑 `/public/service-worker.js` 中的 fetch 事件处理函数。

### 添加预缓存资源

在 `service-worker.js` 的 `PRECACHE_URLS` 数组中添加需要预缓存的资源。

## 常见问题

**Q: 为什么开发环境看不到安装提示？**
A: Service Worker 仅在生产环境启用，使用 `npm run build && npm run preview` 测试。

**Q: 如何更新已安装的 PWA？**
A: Service Worker 会自动检测更新，用户访问时会提示刷新。

**Q: 图标不显示怎么办？**
A: 确保已生成所有尺寸的 PNG 图标并放置在 `public/icons/` 目录。

**Q: 如何卸载 PWA？**
A: Chrome 设置 > 应用 > 找到应用 > 卸载。移动端长按图标 > 卸载。

## 参考资料

- [MDN - Progressive Web Apps](https://developer.mozilla.org/zh-CN/docs/Web/Progressive_web_apps)
- [Google PWA 指南](https://web.dev/progressive-web-apps/)
- [Web App Manifest 规范](https://www.w3.org/TR/appmanifest/)
- [Service Worker API](https://developer.mozilla.org/zh-CN/docs/Web/API/Service_Worker_API)
